...
init()
{
	...
	*/
	...
	max_weapon_num = 149;
	attachment_num = 150;
	for( wi = 0; wi <= max_weapon_num; wi++ ) {
		if ( level.weapon_index_reverse_load == 1 )
			i = max_weapon_num - wi;
		else
			i = wi;
...
dropOffhand()
{
	...
	if ( level.scr_flash_grenades_allowdrop ) {
...
onWeaponDamage( eInflictor, sWeapon, meansOfDeath, damage )
{
	...
	switch( sWeapon ) {
		case "concussion_grenade_mp":
			...
			time = 2 + (level.scr_concussion_grenades_base_time * scale);
